...
import { returnErrors, clearErrors, ideaError } from './error-actions';
...
export const updateIdea = (payload) => async (dispatch) => {
	...
	try {
		...
		if (data) {
			Toast.show({
				type: 'success',
				text1: 'Idea updated successfully.',
				text2: `We're you have created your idea`,
			});
			await dispatch({
				type: UPDATE_IDEA,
				payload: data,
...
export const deleteIdea = (payloadId) => async (dispatch) => {
	...
	try {
		...
		if (data) {
			Toast.show({
				type: 'success',
				text1: 'Idea deleted successfully.',
				text2: `You have delete your idea`,
			});
			await dispatch({
				type: DELETE_IDEA,
				payload: payloadId,
